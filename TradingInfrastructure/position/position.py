...
class Position(object):
    ...
    def __init__(self, action, ticker,
                 init_quantity, init_price,
                 init_commission, bid, ask):
    ...
    def calculate_initial_value(self):
        ...
        """
        Depending upon whether the action was a buy or a sell ("BOT" or "SLD")
        calculate the average bought cost, the total bought cost,
        the average price and the cost basis.

        Finally, calculate the net total with an without commission.
        """
        ...
        if self.action == 'BOT':
            ...
            self.avg_price = ((self.init_price * self.quantity + self.init_commission)/self.quantity).quantize(FIVEPLACES)
            self.cost_basis = (self.quantity * self.avg_price).quantize(TWOPLACES)
    ...
    def update_market_value(self, bid, ask):
        ...
        access to the top of the order book through Interactive Brokers,
        ...
        self.market_value = (self.quantity * midpoint).quantize(TWOPLACES)
    ...
    def transact_shares(self, action, quantity, price, commission):
        ...
        Takes care to update the average bought/sold, total bought/sold,
        the cost basis and PnL calculations, 
        ...
        if action == "BOT":
            self.avg_bot = ((self.avg_bot*self.buys + price*quantity)/(self.buys + quantity)).quantize(FIVEPLACES)
            if self.action != "SLD":
                self.avg_price = (
            ...
            self.avg_sld = ((self.avg_sld*self.sells + price*quantity)/(self.sells + quantity)).quantize(FIVEPLACES)
            if self.action != "BOT":
                self.avg_price = (
                                 (self.avg_price*self.sells + price*quantity - commission)/
                                 (self.sells + quantity)
                                 ).quantize(FIVEPLACES)
            self.sells += quantity
            self.total_sld = (self.sells * self.avg_sld).quantize(TWOPLACES)
