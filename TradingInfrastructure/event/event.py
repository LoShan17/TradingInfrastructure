...
class Event(object):
    """
    Event is a base class providing an interface for all
    subsequent (inherited) events, that will trigger further events
...
class SignalEvent(Event):
    """
    Handles teh event of sending a Signal from a Strategy object.
...
class OrderEvent(Event):
    ...
    def print_order(self):
        ...
        """
        print(
            "Order: Ticker=%s, Action=%s, Quantity=%s" % (self.ticker, self.action, self.quantity)
            )
...
class FillEvent(Event):
    ...
    TODO: Currentlt does not support filling positions at 
