...
from TradingInfrastructure.position.position import Position

class Portfolio(object):
    def __init__(self, price_handler, cash):
        """
        On creation, the Portfolio object contains
        no positions and all values are "reset" to the 
        initial cash, with no PnL - realised or unrealised.
        """
    ...
    def _reset_values(self):
        """
        This is called after every position addition or modification
        It allows the calculations to be carried out "from scratch"
        in order to minimise errors.

        All cash is reset to the initial values and the PnL is set to zero.
        """
    ...
    def _add_position(self, action, ticker,
    ...
    def _modify_position(self, action, ticker,
                         quantity, price, commission):
        """
        Modifies a current Posistion object to the Portfolio
        ...
        if ticker in self.positions:
            ...
            print(
                "Ticker %s not in the current positions list" \
                "Could not modify a current position" % ticker
                )   
    ...
    def transact_position(
        ...
        Hence, this single method wil be called by the
